...
parser.add_argument('--test_name', type=str,default='night', help='the name of the test set')
...
if not os.path.exists("./output/metric/GANfasterRcnn/%s_to_%s/ground_truth"%(opt.train_name,opt.test_name)):
    os.makedirs("./output/metric/GANfasterRcnn/%s_to_%s/ground_truth"%(opt.train_name,opt.test_name))
if not os.path.exists("./output/metric/GANfasterRcnn/%s_to_%s/detection_results"%(opt.train_name,opt.n_experiment,opt.test_name)):
    os.makedirs("./output/metric/GANfasterRcnn/%s_to_%s/detection_results"%(opt.train_name,opt.test_name))
if not os.path.exists("./output/metric/GANfasterRcnn/%s_to_%s/images_optional"%(opt.train_name,opt.test_name)):
    os.makedirs("./output/metric/GANfasterRcnn/%s_to_%s/images_optional"%(opt.train_name,opt.test_name))
...
for i, batch in tqdm(enumerate(dataloader),desc='Processing',total=len(dataloader)):
    ...
    for j in range(len(batch["images"])):
        ...
        save_image(batch["images_orig"][j].data, './output/metric/GANfasterRcnn/%s_to_%s/images_optional/%s.jpg' %(opt.train_name,opt.test_name,name))
        txt1=open('./output/metric/GANfasterRcnn/%s_to_%s/ground_truth/%s.txt'%(opt.train_name,opt.test_name,name), 'w')
        txt2=open('./output/metric/GANfasterRcnn/%s_to_%s/detection_results/%s.txt'%(opt.train_name,opt.test_name,name), 'w')
