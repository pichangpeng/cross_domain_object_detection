...
parser.add_argument('--w', type=int, default=1280, help='width of the image ')
...
if not os.path.exists('output/images/cycleGAN/%d_%d_%d_%s'%(opt.batchSize,opt.h,opt.w,opt.generator_A2B.split('_')[-1][0])):
    os.makedirs('output/images/cycleGAN/%d_%d_%d_%s/real_day/'%(opt.batchSize,opt.h,opt.w,opt.generator_A2B.split('_')[-1][0]))
    os.makedirs('output/images/cycleGAN/%d_%d_%d_%s/fake_night/'%(opt.batchSize,opt.h,opt.w,opt.generator_A2B.split('_')[-1][0]))

for i, batch in enumerate(dataloader):
    # Set model input
    real_A_name=batch['day_name'][0]
    real_A_orig=Variable(input_A.copy_(batch['day_orig']))
    real_A = Variable(input_B.copy_(batch['day']))
